...
pub fn build_builder() -> crate::point_add::B {
    ...
    if inversion::rank5_lifecycle::enabled() {
        assert_eq!(builder.peak_qubits,797+u32::try_from(novelty_pad_qubits).unwrap(),"phase-cargo rank5 whole peak drift");
        if inversion::rank5_lifecycle::candidate_configuration(){
            assert_eq!(builder.current_ops_len(),inversion::rank5_lifecycle::CANDIDATE_OPS+3*novelty_pad_qubits,"Q798 candidate op-count drift");
            assert_eq!(builder.counted_kind_ops[OperationType::CCX as usize]+builder.counted_kind_ops[OperationType::CCZ as usize],inversion::rank5_lifecycle::CANDIDATE_TOFFOLI,"Q798 candidate Toffoli drift");
        }
