...
pub fn add_cuccaro_with_overflow(circ: &mut Circuit, a_ext: &[QReg], b: &[QReg]) {
    let n = b.len();
    assert!(a_ext.len() > n, "a_ext must have n+1 bits for overflow");
    add_cuccaro_with_separate_overflow(circ, &a_ext[..n], b, &a_ext[n]);
}

/// The same adder with an independently owned carry-out lane.
pub fn add_cuccaro_with_separate_overflow(
    circ: &mut Circuit, a: &[QReg], b: &[QReg], ovf: &QReg,
) {
    let n = b.len();
    assert_eq!(a.len(), n);
    if n == 0 {
