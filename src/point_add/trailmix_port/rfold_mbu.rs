...
/// The overflow is measured before evaluating its unchanged phase oracle.
/// Reuse that cleared wire for the carry. Copy the control only when the
/// carry network changes its wire. The comparison window is unchanged.
fn measured_outer_phase(c: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    let phase = c.alloc_bit();
    let carry = &a[256];
    c.hmr(carry, phase);
    c.with_condition(phase, |c| {
        let lo = 256 - COMPARE_TOPK;
        let changed_control = a[lo..256].iter().chain(&b[lo..256])
            .any(|q| q.id() == ctrl.id());
        let owned = changed_control.then(|| c.alloc_qreg("outer.phase.control"));
        let copy = owned.as_ref().unwrap_or(ctrl);
        if changed_control { c.cx(ctrl, copy); }
        c.x(carry);
        for i in lo..256 {
            c.x(&b[i]);
            c.cx(carry, &b[i]);
        ...
        c.x(carry);
        if changed_control { c.cx(ctrl, copy); }
        if let Some(owned) = owned { c.zero_and_free(owned); }
    });
    c.free_bit(phase);
}
...
pub(crate) fn controlled_rfold_window(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], r: &[u8]) {
    if std::env::var("MIDQ_ALL_CONST_FOLDS").ok().as_deref() == Some("1")
        && !a[RFOLD_WINDOW..2 * RFOLD_WINDOW - 1].iter().any(|q| q.id() == ctrl.id())
        && crate::point_add::trailmix_port::inversion::shrunken_pz_state_machine::cell_folds::try_constant_update(
            circ, ctrl, &a[..RFOLD_WINDOW], r, false,
        )
    { return; }
...
fn controlled_outer_int_add(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    // A narrow source gets padding only during integer addition, not during
    // the following fold and phase cleanup. Its 256 data bits never move.
    assert!(b.len() == a.len() || b.len() + 1 == a.len());
    let padding = (b.len() < a.len()).then(|| circ.alloc_qreg("outer_cadd_source.zero"));
    let cap = std::env::var("MIDQ_OUTER_VENT_QCAP").ok()
        .and_then(|value| value.parse::<usize>().ok()).unwrap_or(0);
    if cap == 0 && padding.is_none() {
    ...
    let b_refs: Vec<&QReg> = b.iter().chain(padding.iter()).collect();
    ...
    if let Some(bit) = padding { circ.zero_and_free(bit); }
}
...
pub fn controlled_mod_add_rfold_mbu(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    ...
    if nb == 257 || std::env::var("MIDQ_NARROW_COEFFICIENTS").ok().as_deref() == Some("1") {
    ...
    if std::env::var("MIDQ_MEASURED_OUTER_PHASE").ok().as_deref() == Some("1") {
        measured_outer_phase(circ, ctrl, a, b);
...
//
// requires:
//   result.len() == 257, a.len() in {256, 257}, b.len() == 257
//   result pre == |0…0> (all 257 qubits zero)
//   a[256] == |0> if present, b[256] == |0>
...
pub fn mod_mul_rfold_mbu(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) {
    let n = b.len();
    debug_assert!(matches!(a.len(), 256 | 257));
