...
fn empty_q(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],w1:&[QReg],extra:&[(&QReg,bool)],out:&QReg,helpers:&[QReg]) {
    if super::metadata_muxlease::active("Q796_ENTRY_FACTOR"){
        // Preserve the entire original Boolean predicate: C_high=0, then
        // the special full-Q256 correction where A_high=C_high=0 (r<4).
        let ts=triples();for(r,t)in ts.iter().enumerate(){
            let decoded=rank_programs::C_EQUAL[0].iter().fold(false,|x,&(m,v)|x^((r as u16&m)==v));
            assert_eq!(decoded,t[1]==0);assert_eq!(r<4,t[0]==0&&t[1]==0);
        }
        ...
        let mut cs=extra.to_vec();cs.extend(rank[2..5].iter().chain(a).chain(c).map(|q|(q,false)));cs.push((&w1[2],true));mixed_mcx(circ,&cs,out,helpers);return;
    }
...
pub(super) fn entry_with_support(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,lo:usize,hi:usize) {
    ...
    if super::metadata_muxlease::active("Q796_ENTRY_FACTOR"){
        // E*Sign XOR E*P1 = E*(Sign XOR P1). E does not observe phase bits.
        circ.cx(sign,p1);empty_q(circ,rank,a,c,w1,&[(p1,true)],p2,helpers);circ.cx(sign,p1);
    }else{for q in [sign,p1] {empty_q(circ,rank,a,c,w1,&[(q,true)],p2,helpers);}}
