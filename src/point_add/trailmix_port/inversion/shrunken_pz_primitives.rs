...
#[path = "../../newarith/chunked_compare.rs"]
mod chunked_compare;
...
pub(crate) fn borrow_compare_refs(circ: &mut Circuit, v: &[&QReg], u: &[&QReg], out: &QReg) {
    ...
    if chunked_compare::try_compare(circ, v, u, out) { return; }
...
pub(crate) fn clear_borrow_compare_refs(
    ...
    assert_eq!(u.len(), n);
    if n == 0 { return; }
    if chunked_compare::try_clear_compare(circ, v, u, out) { return; }
...
pub(crate) fn measured_compare_selftest() {
    if std::env::var_os("MIDQ_CHUNK_COMPARE_SELFTEST").is_some() {
        chunked_compare::selftest();
        return;
    }
