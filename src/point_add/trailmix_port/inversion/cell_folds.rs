...
pub(crate) fn try_constant_update(c: &mut Circuit, ctrl: &QReg, target: &[QReg],
    ...
    let Some(chunks) = choose(c, target.len()) else { return false; };
...
fn signed_add(c: &mut Circuit, target: &[QReg], source: &[QReg], sign: &QReg) {
    ...
    if let Some(chunks) = choose(c, 257).filter(|_| std::env::var("MIDQ_CELL_SUM").ok().as_deref() != Some("0")) {
        emit_plan(c, &target[..256], Addend::Word(&source[..256]), Some(overflow), &chunks);
    } else { add_cuccaro_with_separate_overflow(c, &target[..256], &source[..256], overflow); }
