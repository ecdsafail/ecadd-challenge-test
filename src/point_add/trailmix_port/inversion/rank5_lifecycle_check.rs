...
pub fn run(){
 ...
 let core=initialize(&mut circ,dx,&passenger[0]);let initial_map=ids(&core);let initialization=std::mem::take(&mut circ.b.ops);
 ...
 restore_terminal_padding(&mut circ,&mut terminal);let core=rebuild_terminal(&mut circ,terminal,&passenger[0]);let rebuilt_map=ids(&core);let rebuild=std::mem::take(&mut circ.b.ops);let dx=finish(&mut circ,core);let output_ids:Vec<_>=dx.iter().map(|q|q.id()as usize).collect();let finishing=std::mem::take(&mut circ.b.ops);
 assert_eq!(circ.b.active_qubits,513);assert_eq!(circ.b.peak_qubits,797);assert_eq!(circ.b.next_qubit,797);let owned=circ.b.next_qubit as usize;let bits=circ.b.next_bit as usize;
 ...
 for inverse in [false,true]{let mapping=if inverse{&rebuilt_map}else{&initial_map};
  for z in 0..26{let block=if inverse{25-z}else{z};let templates:Vec<_>=(0..4).map(|j|remap(template(block,j),mapping,&passenger,inverse)).collect();let first=block*64;let end=(first+64).min(1616);
   ...
   for(batch,sim)in sims.iter().enumerate(){for q in &passenger[1..]{assert_eq!(sim.qubits[q.id()as usize],initial[batch][q.id()as usize],"passenger inverse={inverse} block={block}");}}eprintln!("RANK5_LIFECYCLE_BLOCK inverse={inverse} block={block} PASS");
  }
  if !inverse{for batch in 0..batches{
   ...
   for q in 0..owned{if !terminal_ids.contains(&q)&&!passenger.iter().any(|p|p.id()as usize==q){released[q]=0;}}released[passenger[0].id()as usize]=initial[batch][passenger[0].id()as usize];compare(&sims[batch],&released,"released terminal",batch);
 ...
 eprintln!("RANK5_LIFECYCLE_PASS inputs={count} lanes={} peak_qubits=797 forward_ops={forward_ops} forward_T={forward_t} lifecycle_ops={lifecycle_ops}; actual init/full forward/release/independent inverse/sign/rebuild/full inverse/finish, all256 passengers restored, every reset checked; NOT whole point-add or official9024",64*batches);
}
