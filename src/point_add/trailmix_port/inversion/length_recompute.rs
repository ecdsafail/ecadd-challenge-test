...
/// XOR form of eight-bit comparison, used only where callers apply each cube
/// as a target toggle. The terms need not be disjoint; their parity is exact.
pub(super) fn threshold_xor_cubes(width:usize,bound:usize,above:bool)->Vec<Vec<(usize,bool)>> {
    static ENABLED:std::sync::OnceLock<bool>=std::sync::OnceLock::new();
    if width!=8 || !*ENABLED.get_or_init(||std::env::var("LOWQ_THRESHOLD_ESOP").ok().as_deref()==Some("1")) {
        return if above {above_cubes(width,bound)} else {below_cubes(width,bound)};
    }
    if above && bound>=255 {return Vec::new();}
    let program=if above {super::runtime_threshold_programs::ABOVE[bound]} else {super::runtime_threshold_programs::BELOW[bound.min(256)]};
    let result:Vec<Vec<(usize,bool)>>=program.iter().map(|&(mask,value)|
        (0..8).rev().filter(|&i|(mask>>i)&1!=0).map(|i|(i,(value>>i)&1!=0)).collect()).collect();
    result
}
