...
const STREAM_CX_PER_TRAVERSAL: usize = 25_909_574;
// Includes the two emitted CCX gates for every clean-C3X MBU marker.
const STREAM_CCX_PER_TRAVERSAL: usize = 35_987_307;
const STREAM_HMR_PER_TRAVERSAL: usize = 5_183_548;
const STREAM_CZ_PER_TRAVERSAL: usize = 5_183_548;
...
fn local_wires<'a>(core: &'a Core, passenger: &'a [QReg]) -> Vec<&'a QReg> {
    ...
    let physical: std::collections::HashSet<u32> = wires.iter().map(|q| q.id()).collect();
    assert_eq!(physical.len(), LOCAL_WIDTH, "maximal cache requires injective physical map");
    wires
}
