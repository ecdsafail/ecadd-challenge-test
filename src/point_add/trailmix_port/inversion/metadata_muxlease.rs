...
fn predicate_swap(circ:&mut Circuit,rank:&[QReg],axis:usize,bit:usize,left:&QReg,right:&QReg,helpers:&[QReg]) {
    let anf:Vec<_>=triples().iter().map(|t|(t[axis]>>bit)&1!=0).collect();
    truth_swap(circ,&rank.iter().collect::<Vec<_>>(),anf,left,right,helpers);
}
fn truth_swap(circ:&mut Circuit,controls:&[&QReg],mut anf:Vec<bool>,left:&QReg,right:&QReg,helpers:&[QReg]) {
