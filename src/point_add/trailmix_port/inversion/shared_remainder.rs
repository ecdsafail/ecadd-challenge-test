...
fn below<'a>(r:&'a[QReg],bound:usize)->Vec<Cube<'a>> {
    threshold_xor_cubes(r.len(),bound,false).iter().map(|c|c.iter().map(|&(i,v)|(&r[i],v)).collect()).collect()
}
fn above<'a>(r:&'a[QReg],bound:usize)->Vec<Cube<'a>> {
    threshold_xor_cubes(r.len(),bound,true).iter().map(|c|c.iter().map(|&(i,v)|(&r[i],v)).collect()).collect()
}
