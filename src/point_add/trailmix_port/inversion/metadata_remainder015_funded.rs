...
impl Scan<'_> {
    ...
    fn lo(&self,circ:&mut Circuit,i:usize,group:isize,data:&[&QReg],out:&QReg,enabled:bool) {
        ...
        if super::metadata_muxlease::active("Q799_XOR_LO") {
            ...
            let mut cs=vec![(self.guard,true),(self.hs,true),(&self.c[0],old_c0)];cs.extend((0..4).map(|b|(&self.sm[b],value>>(b+2)&1!=0)));cs.extend(data.iter().map(|&q|(q,true)));if enabled{if let Some(s)=self.restore{cs.push((s,self.restore_positive));}}mixed_mcx(circ,&cs,out,self.helpers);
            circ.cx(&self.a[0],&self.c[0]);
        } else {for av in [false,true] {
            let mut cs=vec![(self.guard,true),(self.hs,true),(&self.a[0],av),(&self.c[0],av^old_c0)];cs.extend((0..4).map(|b|(&self.sm[b],value>>(b+2)&1!=0)));cs.extend(data.iter().map(|&q|(q,true)));if enabled{if let Some(s)=self.restore{cs.push((s,self.restore_positive));}}mixed_mcx(circ,&cs,out,self.helpers);
        }}
...
pub(super) fn signless(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize) {
    assert!(helpers.len()>=24);let start=circ.b.ops.len();
