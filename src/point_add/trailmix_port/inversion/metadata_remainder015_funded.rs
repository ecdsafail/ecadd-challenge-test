...
struct Scan<'a> {rank:&'a[QReg],a:&'a[QReg],c:&'a[QReg],sm:&'a[QReg],mask:&'a QReg,guard:&'a QReg,hs:&'a QReg,ha:&'a QReg,helpers:&'a[QReg],j:usize,restore:Option<&'a QReg>,support_end:usize}
impl Scan<'_> {
    ...
    fn lo(&self,circ:&mut Circuit,i:usize,group:isize,data:&[&QReg],out:&QReg,enabled:bool) {
        ...
        if super::metadata_muxlease::active("Q799_XOR_LO") {
            ...
            let mut cs=vec![(self.guard,true),(self.hs,true),(&self.c[0],old_c0)];cs.extend((0..4).map(|b|(&self.sm[b],value>>(b+2)&1!=0)));cs.extend(data.iter().map(|&q|(q,true)));if enabled{if let Some(s)=self.restore{cs.push((s,false));}}mixed_mcx(circ,&cs,out,self.helpers);
            circ.cx(&self.a[0],&self.c[0]);
        } else {for av in [false,true] {
            let mut cs=vec![(self.guard,true),(self.hs,true),(&self.a[0],av),(&self.c[0],av^old_c0)];cs.extend((0..4).map(|b|(&self.sm[b],value>>(b+2)&1!=0)));cs.extend(data.iter().map(|&q|(q,true)));if enabled{if let Some(s)=self.restore{cs.push((s,false));}}mixed_mcx(circ,&cs,out,self.helpers);
        }}
    ...
    fn scan(&self,circ:&mut Circuit,source:&[QReg],target:&[QReg],sign:Option<&QReg>,role:usize,reverse:bool) {
        ...
        for z in 0..self.support_end {let i=if reverse {self.support_end-1-z}else{z};let group=(if i<=255{(((i+1)%256)/64)as isize}else{-1},if i<=256{((256-i)/64)as isize}else{-1});
            ...
            match role {
                ...
                3=>{
                    if i+1<self.support_end {self.data(circ,&[a[i],b[i]],a[i+1]);}
                    if let Some(s)=sign {self.top(circ,i,&[a[i],b[i]],s,false,true);}
                },
                4=>if i+1<self.support_end {self.data(circ,&[a[i+1]],b[i+1]);self.data(circ,&[a[i],b[i]],a[i+1]);},
...
pub(super) fn phase01_with_support(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize){
    ...
    let mut scan=Scan{rank,a,c,sm,mask:p1,guard:g,hs:p2,ha,helpers:dirty,j,restore:None,support_end};
