...
fn initialize(circ:&mut Circuit,mut dx:Vec<QReg>)->Core {
    ...
    Core {rank:circ.alloc_qreg_bits("rank5.rank",5),a:circ.alloc_qreg_bits("rank5.a",6),c:circ.alloc_qreg_bits("rank5.c",6),sm:circ.alloc_qreg_bits("rank5.sm",4),phase1:circ.alloc_qreg("rank5.P1"),phase2:circ.alloc_qreg("rank5.P2"),iteration,work1,work2:dx}
}
fn release_terminal(circ:&mut Circuit,mut core:Core)->Terminal {
    ...
    circ.zero_and_free(core.phase1);circ.zero_and_free(core.phase2);
...
fn rebuild_terminal(circ:&mut Circuit,mut terminal:Terminal)->Core {
    ...
    Core {rank,a,c:terminal.history,sm,phase1:circ.alloc_qreg("rank5.P1.rebuilt"),phase2:circ.alloc_qreg("rank5.P2.rebuilt"),iteration:terminal.iteration,work1,work2:terminal.work2}
}
...
fn template(block:usize,j:usize)->Vec<Op> {
    ...
    let p1=circ.alloc_qreg("p1");let p2=circ.alloc_qreg("p2");let iter=circ.alloc_qreg("iter");let w1=circ.alloc_qreg_bits("w1",259);let w2=circ.alloc_qreg_bits("w2",259);let helpers=circ.alloc_qreg_bits("borrowed",24);assert_eq!(circ.b.next_qubit,566);
    super::q798_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&iter,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,566);
...
fn remap(mut ops:Vec<Op>,core_ids:&[usize],passenger:&[QReg],inverse:bool)->Vec<Op> {
    assert_eq!(core_ids.len(),542);assert!(passenger.len()>=24);let mut mapping=core_ids.to_vec();mapping.extend(passenger[..24].iter().map(|q|q.id()as usize));
    let mut unique=mapping.clone();unique.sort_unstable();unique.dedup();assert_eq!(unique.len(),566);
...
pub(crate) const CANDIDATE_OPS:usize=2_218_070_988;
pub(crate) const CANDIDATE_TOFFOLI:usize=1_791_596_215;
